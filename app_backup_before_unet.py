...
st.title(APP_TITLE)
st.caption("전처리 이미지·최종 mask 업로드 · 검토 · 품질 라벨 · handoff export")
...
with upload_tab:
    st.subheader(selected_specimen_display or '-')
    if not selected_specimen:
        ...
        st.info("전처리 이미지 업로드는 rule-based 1차 전처리까지 끝난 기준 이미지 ZIP 1개만 받습니다. 이 이미지는 LabelMe 라벨링·U-Net 학습·최종 mask와 1:1로 대응되는 기준 이미지입니다. 새 ZIP을 적용하면 기존 이미지는 전부 삭제되고 교체됩니다.")
        ...
        with st.expander("업로드 항목 설명", expanded=False):
            ...
            st.markdown("- **예측 Mask 업로드(선택)**: U-Net이 처음 생성한 원본 예측 결과. 최종 mask와 비교 검토가 필요할 때만 업로드하면 됩니다.")
            ...
            if up_preds:
                ...
                if st.button("예측 Mask 업로드 적용", key=f"apply_preds_{selected_specimen}_{st.session_state[pred_nonce_key]}"):
                    with st.spinner("예측 mask 업로드 중..."):
                        ...
                        st.cache_data.clear()
                    st.session_state[flash_key] = f"예측 mask 저장 완료(비교용): {cnt}장"
...
with export_tab:
    ...
    if truth_df.empty:
        ...
        st.caption("검토/추출 전용입니다. 입력/수정은 Quality Label 탭에서 진행하세요. handoff 패키지에는 전체 CSV와 함께 시편별 시트로 분리된 dataset_manifest_all.xlsx도 포함됩니다.")
