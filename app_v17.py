        ...
        if dataset_df.empty:
            ...
            with fume_tab:
                ...
                if matched_fume_frame is not None and matched_fume_frame.exists():
                    ...
                    review_fume_video = get_review_video_path(fume_bundle)
                    if not render_video_preview_player(review_fume_video, "Fume video"):
                        st.info("Fume 영상을 샘플 프레임 플레이어로 불러오지 못했습니다.")
                    else:
                        with open(review_fume_video, "rb") as _vf:
                            st.download_button("Fume 영상 다운로드", _vf.read(), file_name=review_fume_video.name, mime=get_video_mime_type(review_fume_video), key=f"dl_fume_{selected_specimen}_{item_id}")
            ...
            with spatter_tab:
                ...
                if matched_spatter_frame is not None and matched_spatter_frame.exists():
                    ...
                    review_spatter_video = get_review_video_path(spatter_bundle)
                    if not render_video_preview_player(review_spatter_video, "Spatter video"):
                        st.info("Spatter 영상을 샘플 프레임 플레이어로 불러오지 못했습니다.")
                    else:
                        with open(review_spatter_video, "rb") as _vs:
                            st.download_button("Spatter 영상 다운로드", _vs.read(), file_name=review_spatter_video.name, mime=get_video_mime_type(review_spatter_video), key=f"dl_spatter_{selected_specimen}_{item_id}")
